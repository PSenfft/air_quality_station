...
async fn sense_task(
    mut ens160: Ens160<I2cDevice<'static, NoopRawMutex, I2c<'static, Async>>, Delay>,
) {
    let tx = CO2.sender();
        ...
        if let Ok(status) = ens160.get_status().await {
            if status.new_data_ready() {
                if let Ok(measurements) = ens160.get_measurements().await {
                    ...
                    let co2: u16 = measurements.co2eq_ppm.get_value();
                    tx.send(co2);
                } else {
            ...
            error!("Failed to get ENS160 status.");
        }
        Timer::after(Duration::from_secs(2)).await;
    }
}

pub async fn start_sense(
    spawner: Spawner,
    i2c_dev: I2cDevice<'static, NoopRawMutex, I2c<'static, Async>>,
) -> Result<(), &'static str> {
    let ens160 = init_ens(i2c_dev).await?;
    spawner
        .spawn(sense_task(ens160))
        .map_err(|_| "Faild to spawn sense task!")
}

async fn init_ens(
    i2c_dev: I2cDevice<'static, NoopRawMutex, I2c<'static, Async>>,
) -> Result<Ens160<I2cDevice<'static, NoopRawMutex, I2c<'static, Async>>, Delay>, &'static str> {
